...
func loadConfByClusterName(clusterName, userDefinedConfPath string, useConsul bool) (servers []string, err error) {
	...
	// get from ssconf if useConsul is false
	if !useConsul {
	...
	// get from redis
	if len(servers) == 0 || err != nil {
		servers, err = serviceDiscoveryByRedis(consulClusterName)
	}
...
func serviceDiscoveryByConsul(clusterName string) ([]string, error) {
	...
	// prefer local idc, if nil then try another idc
	var insts []*Instance = nil
	insts = consulAgent.Lookup(LocalIDC())
	if insts == nil {
		if LocalIDC() == LF_IDC {
			insts = consulAgent.Lookup(HY_IDC)
		} else {
			insts = consulAgent.Lookup(LF_IDC)
		}
	}
	if insts == nil {
