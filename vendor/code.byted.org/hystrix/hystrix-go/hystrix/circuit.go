...
func (circuit *NoOpCircuitBreaker) ExecutorPool() *executorPool {
...
var dingTalkAlarm = alarm.NewDingTalkAlarm("8", "401F4DAC3F8A4804BE2A2E999DA83CF7")
...
func (circuit *CircuitBreaker) setOpen() {
	...
	now := time.Now().UnixNano()
	if now > circuit.lastAlarmedTime+getSettings(circuit.name).DingTalkAlarmSleepWindow.Nanoseconds() {
		circuit.lastAlarmedTime = now
		circuit.alarmed = true
		go dingTalkAlarm.SendDingTalkAlarms(getSettings(circuit.name).DingTalkAlarmChannels,
			fmt.Sprintf("circuit %s opened, request sum %v, error percent %v", circuit.name,
				circuit.metrics.Requests().Sum(time.Now()),
				circuit.metrics.ErrorPercent(time.Now())))
	}
}
...
func (circuit *CircuitBreaker) setClose() {
	...
	if circuit.alarmed {
		circuit.alarmed = false
		go dingTalkAlarm.SendDingTalkAlarms(getSettings(circuit.name).DingTalkAlarmChannels,
			fmt.Sprintf("circuit %s closed, back to normal now.", circuit.name))
	}
}
...
func (circuit *CircuitBreaker) ExecutorPool() *executorPool {
	return circuit.executorPool
}
