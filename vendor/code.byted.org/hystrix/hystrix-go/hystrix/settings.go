...
const (
	...
	// DefaultDingTalkAlarmChannel is the default channel to send hystrix alarm
	DefaultDingTalkAlarmChannel = "47" // Own by webarch, wangfenjin
	// DefaultDingTalkAlarmSleepWindow is how long, in minutes, to wait after send an alarm
	DefaultDingTalkAlarmSleepWindow = 10
)
...
type settings struct {
	...
	DingTalkAlarmChannels    []string
	DingTalkAlarmSleepWindow time.Duration
}
...
type CommandConfig struct {
	...
	DingTalkAlarmChannels    []string `json:"ding_talk_alarm_channels" yaml:"ding_talk_alarm_channels"`
	DingTalkAlarmSleepWindow int      `json:"ding_talk_alarm_sleep_window" yaml:"ding_talk_alarm_sleep_window"`
}
...
func ConfigureCommand(name string, config CommandConfig) {
	...
	alarmChannels := []string{DefaultDingTalkAlarmChannel}
	if len(config.DingTalkAlarmChannels) > 0 {
		alarmChannels = append(alarmChannels, config.DingTalkAlarmChannels...)
	}

	alarmSleepWindow := DefaultDingTalkAlarmSleepWindow
	if config.DingTalkAlarmSleepWindow != 0 {
		alarmSleepWindow = config.DingTalkAlarmSleepWindow
	}
	...
	circuitSettings[name] = &settings{
		...
		DingTalkAlarmChannels:    alarmChannels,
		DingTalkAlarmSleepWindow: time.Duration(alarmSleepWindow) * time.Minute,
	}
}
