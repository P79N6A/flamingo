...
type Options struct {
	...
	ShouldTrip     TripFunc // can be changed
	MaxConcurrency int64    // can be changed
	sync.RWMutex            // for change ShouldTrip
...
func NewBreaker(options *Options) (*Breaker, error) {
	if options == nil || options.ShouldTrip == nil {
		return nil, errors.New("No Trip Function")
	}
...
// Done decreases the concurrency counter by one
func (b *Breaker) Done() {
	b.Lock()
	b.concurrencyNow--
	b.Unlock()
...
// IsAllowedWithMaxCon .
func (b *Breaker) IsAllowedWithMaxCon(maxCon int64) bool {
	return b.isAllowed(maxCon)
}

// IsAllowed .
func (b *Breaker) IsAllowed() bool {
	return b.isAllowed(b.options.MaxConcurrency)
}

// IsAllowed .
func (b *Breaker) isAllowed(maxCon int64) bool {
	b.Lock()
	// first, check the max concurrency
	if maxCon > 0 && b.concurrencyNow+1 > maxCon {
		b.Unlock()
		return false
	}
	...
	// third, increase the coucurrency counter
	b.concurrencyNow++
...
// Concurrency .
func (b *Breaker) Concurrency() int64 {
	b.Lock()
	c := b.concurrencyNow
	b.Unlock()
	return c
