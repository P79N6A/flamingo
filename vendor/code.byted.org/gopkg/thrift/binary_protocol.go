...
const (
	// 15MB, the framed transport default limit size is 15MB too.
	SAFE_BUFFER_SIZE_LIM = 15 * (1 << 20)
)

var (
	limitReadBytes int64
	doOnceLimit    *sync.Once = &sync.Once{}
)

var (
	SafeBufferError = NewTProtocolException(errors.New("Buffer is larger than the limitReadBytes"))
)
...
func init() {
	limitReadBytes = SAFE_BUFFER_SIZE_LIM
}

// SetLimitReadBytes limit readString or Binary data, default is 15MB.
func SetLimitReadBytes(size int64) {
	doOnceLimit.Do(func() {
		limitReadBytes = size
	})
}
...
func (p *TBinaryProtocol) readStringBody(size int) (value string, err error) {
	...
	_, e := io.CopyN(buffer, p.trans, int64(size))
	return buffer.String(), NewTProtocolException(e)
}
